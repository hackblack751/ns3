...
//
  uint16_t port1 = 8000; 
  // Creat a UdpServer running on port 8000
...
//
  ...
  // Set up parameters for the UdpClient
...
// Create one udpClient application on node 1.
...
//Set up
  ...
  // Install the UdpClient on node0
  apps = client2.Install (nodes.Get (1));
  // Specify the experimental time
  apps.Start (Seconds (2.0));
  apps.Stop (Seconds (10.0));
